...
const OTM_KEY = "5ae2e3f221c38a28845f05b6780df48d720d8fb5dccacec8f8944769"; // 👈 paste your key here
...
const metaBox = {
  ...
  border:"1px solid rgba(255,255,255,0.15)",borderRadius:10,padding:"8px 12px",color:"#fff",
};
...
function MapModal({ onClose, onPinConfirm }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width:"100%",maxWidth:720,background:"#0f2027",
        ...
        <div style={{padding:"12px 18px",borderTop:"1px solid rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
          <span style={{fontSize:14,color: pinned ? "#7dd3fc" : "rgba(255,255,255,0.4)"}}>
            {pinned
              ? <><i className="ti ti-map-pin" style={{marginRight:6}}></i>{pinned.display}</>
              : "No location pinned yet"}
          </span>
          <button
            disabled={!pinned}
            onClick={() => { onPinConfirm(pinned); onClose(); }}
            style={{
              padding:"9px 22px",borderRadius:10,border:"none",cursor: pinned ? "pointer" : "not-allowed",
              background: pinned ? "#0ea5e9" : "rgba(255,255,255,0.1)",
              color: pinned ? "#fff" : "rgba(255,255,255,0.3)",
              fontWeight:600,fontSize:14,
              boxShadow: pinned ? "0 4px 16px rgba(14,165,233,0.4)" : "none",
              transition:"all 0.2s",
            }}>
...
export default function App() {
  ...
  const fetchPlaces = async (lat, lon) => {
    ...
    try {
      const r1 = await fetch(`https://api.opentripmap.com/0.1/en/places/radius?radius=5000&lon=${lon}&lat=${lat}&kinds=interesting_places&limit=6&format=json&apikey=${OTM_KEY}`);
      const data = await r1.json();
      const items = Array.isArray(data) ? data : (data.features || []);

      const detailed = await Promise.all(
        items.slice(0, 6).map(async (p) => {
          ...
          return {
            ...
            kinds: d.kinds || "",
            desc: d.wikipedia_extracts?.text?.slice(0, 100) || d.info?.descr?.slice(0, 100) || "A notable place worth visiting.",
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f2027,#203a43,#2c5364)", fontFamily:"'Segoe UI',sans-serif", color:"#fff", padding:"0 0 3rem" }}>
      ...
      {/* Hero / Search */}
      <div style={{ background:"linear-gradient(180deg,rgba(0,0,0,0.45) 0%,transparent 100%)", padding:"2.5rem 1rem 2rem", textAlign:"center" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginBottom:6 }}>
          <i className="ti ti-world" style={{ fontSize:28, color:"#7dd3fc" }}></i>
          <span style={{ fontSize:26, fontWeight:600, letterSpacing:0.5 }}>TripWeather</span>
        </div>
        <p style={{ fontSize:14, color:"rgba(255,255,255,0.65)", margin:"0 0 1.5rem" }}>Search any destination to plan your trip around the weather</p>
        ...
        <div style={{ maxWidth:600, margin:"0 auto", display:"flex", gap:8, position:"relative" }}>
          <div style={{ flex:1, position:"relative" }}>
            ...
            <input
              ...
              style={{
                width:"100%", boxSizing:"border-box", height:52, paddingLeft:44, paddingRight:16,
                fontSize:15, borderRadius:12, background:"rgba(255,255,255,0.95)",
            ...
            {suggestions.length > 0 && (
              <div style={{
                ...
                background:"rgba(15,32,43,0.97)", backdropFilter:"blur(16px)", WebkitBackdropFilter:"blur(16px)",
                border:"1px solid rgba(255,255,255,0.15)", borderRadius:12, zIndex:20, overflow:"hidden",
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"0 1rem" }}>
        ...
        {loading && <p style={{ textAlign:"center", color:"rgba(255,255,255,0.6)", fontSize:14, marginTop:"2rem" }}><i className="ti ti-loader" style={{ fontSize:18 }}></i> Fetching weather...</p>}
        ...
        {weather && cur && daily && (
          <div>
            ...
            <div style={{ marginBottom:12 }}>
              <p style={{ fontSize:13, fontWeight:500, color:"rgba(255,255,255,0.6)", marginBottom:8, textTransform:"uppercase", letterSpacing:1 }}>5-day forecast</p>
            ...
            {/* Place suggestions */}
            <div>
              <p style={{ fontSize:13, fontWeight:500, color:"rgba(255,255,255,0.6)", marginBottom:8, textTransform:"uppercase", letterSpacing:1 }}>
                ...
                {places.map((p,i) => (
                  <div key={i} style={{ ...card, padding:"12px 14px", marginBottom:0 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                      <i className={`ti ${p.icon}`} style={{ fontSize:18, color:"#7dd3fc" }}></i>
                      <span style={{ fontWeight:600, fontSize:14 }}>{p.name}</span>
                    </div>
                    <p style={{ margin:0, fontSize:13, color:"rgba(255,255,255,0.7)", lineHeight:1.5 }}>{p.desc}</p>
                  </div>
                ))}
              </div>
            </div>
